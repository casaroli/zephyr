...
static size_t __ramfunc psram_init(const struct memc_rp2350_psram_config *dev_cfg)
{
	...
	qmi_hw->m[1].timing = 1 << QMI_M1_TIMING_COOLDOWN_LSB |
			      ...
			      2 << QMI_M1_TIMING_MIN_DESELECT_LSB | 2 << QMI_M1_TIMING_RXDELAY_LSB |
			      1 << QMI_M1_TIMING_CLKDIV_LSB;
#else
	qmi_hw->m[1].timing = dev_cfg->cooldown << QMI_M1_TIMING_COOLDOWN_LSB |
	...
	qmi_hw->m[1].rfmt = dev_cfg->read.dtr << QMI_M1_RFMT_DTR_LSB |
	...
	qmi_hw->m[1].rcmd = dev_cfg->read.suffix << QMI_M1_RCMD_SUFFIX_LSB |
			    dev_cfg->read.prefix << QMI_M1_RCMD_PREFIX_LSB;

	qmi_hw->m[1].wfmt = dev_cfg->write.dtr << QMI_M1_WFMT_DTR_LSB |
	...
	qmi_hw->m[1].wcmd = dev_cfg->write.suffix << QMI_M1_WCMD_SUFFIX_LSB |
...
static int memc_rp23xx_psram_init(const struct device *dev)
{
	...
	LOG_INF("max frequency: %u", dev_cfg->max_frequency);
	size_t size;
	size = psram_init(dev_cfg);

	LOG_INF("intialized psram of size %llu\n", (uintmax_t)size);
	...
	LOG_INF("intialized external shared multi heap of size %llu\n", (uintmax_t)size);
...
DEVICE_DT_INST_DEFINE(0, &memc_rp23xx_psram_init, NULL, NULL, &memc_rp2350_config, POST_KERNEL,

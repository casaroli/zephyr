...
int airoc_gpio_port_get_raw(const struct device *port, gpio_port_value_t *value) {
	whd_result_t res = whd_wifi_get_iovar_buffer(airoc_sta_if, "ccgpioin", (uint8_t *)value, 4);
	if (res != WHD_SUCCESS) {
		return -EIO;
	}
...
int airoc_gpio_port_set_masked_raw(const struct device *port, gpio_port_pins_t mask, gpio_port_value_t value) {
    uint8_t buffer[8];
    memcpy(buffer, (void *)&mask, 4);
    memcpy(buffer+4, (void *)&value, 4);
    whd_result_t res = whd_wifi_set_iovar_buffer(airoc_sta_if, "gpioout", buffer, 8);
    if (res != WHD_SUCCESS) {
        return -EIO;
    }
...
DEVICE_DT_DEFINE(DT_INST_CHILD(0, gpio), airoc_gpio_init, NULL, NULL, \
			      NULL, POST_KERNEL, CONFIG_WIFI_INIT_PRIORITY,  \
			      &api_table);

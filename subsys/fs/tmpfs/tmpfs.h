...
#define tmpfs_lock(fs)              k_mutex_lock(&fs->tfs_lock, K_FOREVER)
#define tmpfs_lock_object(to)       k_mutex_lock(&to->to_lock, K_FOREVER)
#define tmpfs_lock_file(tfo)        k_mutex_lock(&tfo->tfo_lock, K_FOREVER)
#define tmpfs_lock_directory(tdo)   k_mutex_lock(&tdo->tdo_lock, K_FOREVER)
#define tmpfs_unlock(fs)            k_mutex_unlock(&fs->tfs_lock)
#define tmpfs_unlock_object(to)     k_mutex_unlock(&to->to_lock)
#define tmpfs_unlock_file(tfo)      k_mutex_unlock(&tfo->tfo_lock)
#define tmpfs_unlock_directory(tdo) k_mutex_unlock(&tdo->tdo_lock)
...
struct tmpfs_object_s {
	...
	struct tmpfs_directory_s *to_parent;
};
...
struct tmpfs_directory_s {
	...
	struct tmpfs_dirent_s *tdo_entry;
};
...
struct tmpfs_file_s {
	...
	uint8_t *tfo_data; /* File data starts here */
};
...
typedef int (*tmpfs_foreach_t)(struct tmpfs_directory_s *tdo, unsigned int index, void *arg);
